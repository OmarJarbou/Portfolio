    ...
    return (
        <motion.div {...pageTransition} className="min-h-screen pt-20 pb-32">
            <div className="max-w-4xl mx-auto px-4 sm:px-6">
                ...
                <button
                    onClick={() => navigate(-1)}
                    className="flex items-center gap-2 text-slate-500 hover:text-white transition-colors duration-200 mb-8 group"
                >
                ...
                <div className="mb-8">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        {project.featured && (
                            <span className="px-3 py-1 rounded-full text-xs font-semibold bg-primary-500/15 text-primary-300 border border-primary-500/25">
                                Featured
                            </span>
                        )}
                    </div>
                        ...
                        {allImages.length > 1 && (
                            <>
                                <button
                                    ...
                                    className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-xl glass flex items-center justify-center hover:bg-white/20 transition-colors"
                                >
                                    <ChevronLeft className="w-5 h-5" />
                                </button>
                                <button
                                    onClick={() => setActiveImage((activeImage + 1) % allImages.length)}
                                    className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-xl glass flex items-center justify-center hover:bg-white/20 transition-colors"
                                >
                ...
                {/* Long Description */}
                <div className="glass rounded-2xl p-6 sm:p-8 border border-white/5 mb-8">
                ...
                {project.challenges?.length > 0 && (
                    <div className="grid sm:grid-cols-2 gap-6">
                        <div className="glass rounded-2xl p-6 border border-red-500/10 bg-gradient-to-br from-red-500/5 to-transparent">
                            ...
                            <ul className="space-y-3">
                                {project.challenges.map((c, i) => (
                                    <li key={i} className="flex gap-3 text-sm text-slate-400 leading-relaxed">
                                        <span className="flex-shrink-0 w-5 h-5 rounded-full bg-red-500/15 text-red-400 text-xs flex items-center justify-center font-bold mt-0.5">{i + 1}</span>
                                        {c}
                                    </li>
                                ))}
                            </ul>
                        </div>
                        <div className="glass rounded-2xl p-6 border border-emerald-500/10 bg-gradient-to-br from-emerald-500/5 to-transparent">
                            <h2 className="text-base font-bold text-white mb-4 flex items-center gap-2">
                            ...
                            <ul className="space-y-3">
                                {project.solutions.map((s, i) => (
                                    <li key={i} className="flex gap-3 text-sm text-slate-400 leading-relaxed">
                                        <span className="flex-shrink-0 w-5 h-5 rounded-full bg-emerald-500/15 text-emerald-400 text-xs flex items-center justify-center font-bold mt-0.5">✓</span>
                                        {s}
                                    </li>
                                ))}
                            </ul>
